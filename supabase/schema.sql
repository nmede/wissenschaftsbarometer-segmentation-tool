...
begin
  return jsonb_build_object(
    ...
    'first_response_at', (select min(created_at) from responses where survey_id = p_survey_id),
    'median_duration_sec', (
      select round(percentile_cont(0.5) within group (order by duration_sec)::numeric, 0)
      from responses where survey_id = p_survey_id and duration_sec is not null),
    'avg_age', (
      select round(avg(extract(year from now())::int - age_year)::numeric, 1)
      from responses where survey_id = p_survey_id and age_year is not null),
    'means_by_segment', coalesce((
      select jsonb_agg(jsonb_build_object('k', k, 'segment', segment, 'm', m, 'n', n))
      from (
        select e.key k, r.segment, round(avg(e.value::numeric), 2) m, count(*) n
        from responses r, jsonb_each_text(r.answers) e
        where r.survey_id = p_survey_id and r.segment is not null
          and e.key in ('F2003','F0505','POL','REL')
          and e.value ~ '^[0-9]+$' and (e.value)::int between 1 and 7
        group by e.key, r.segment
      ) mm), '[]'::jsonb),
    'idxsl_by_segment', coalesce((
      select jsonb_object_agg(segment::text, jsonb_build_object('m', m, 'n', n))
      from (select segment, round(avg(idx_sl)::numeric, 2) m, count(idx_sl) n
            from responses where survey_id = p_survey_id and segment is not null and idx_sl is not null
            group by segment) sl), '{}'::jsonb),
    'avg_certainty', (
    ...
    'quality_by_instrument', coalesce((
      select jsonb_agg(jsonb_build_object('instrument', instrument, 'n', n,
                                          'certainty', certainty, 'clarity', clarity)
                       order by instrument)
      from (
        select coalesce(r.instrument, '20') as instrument, count(*) n,
               round(avg(q.top)::numeric, 3) certainty,
               round(avg(q.top - q.p2)::numeric, 3) clarity
        from responses r
        cross join lateral (
          select max(v) top,
                 (select v2 from unnest(array[r.prob_sciencephiles, r.prob_critically_interested,
                                              r.prob_passive_supporters, r.prob_disengaged]) v2
                  order by v2 desc offset 1 limit 1) as p2
          from unnest(array[r.prob_sciencephiles, r.prob_critically_interested,
                            r.prob_passive_supporters, r.prob_disengaged]) v
        ) q
        where r.survey_id = p_survey_id and r.segment is not null
          and r.prob_sciencephiles is not null
        group by coalesce(r.instrument, '20')
      ) qq), '[]'::jsonb),
...
-- Create a customer or reset their access code from the hub (master code
-- gated). The code is transmitted once and stored hashed; it cannot be
-- read back later.
create or replace function public.admin_set_survey(p_admin_code text, p_survey_id text, p_name text, p_code text)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  if not admin_check(p_admin_code) then return null; end if;
  if p_survey_id is null or length(trim(p_survey_id)) < 3 or p_code is null or length(p_code) < 6 then
    return false;
  end if;
  insert into surveys (survey_id, display_name, code_hash)
  values (trim(p_survey_id), p_name, extensions.crypt(p_code, extensions.gen_salt('bf')))
  on conflict (survey_id)
  do update set display_name = coalesce(excluded.display_name, surveys.display_name),
                code_hash = excluded.code_hash;
  return true;
end;
$$;
grant execute on function public.admin_set_survey(text, text, text, text) to anon;

-- Delete a customer entirely (responses + survey row). Master-code gated;
-- the hub asks the operator to re-enter the master code for this action.
create or replace function public.admin_delete_survey(p_admin_code text, p_survey_id text)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from responses where survey_id = p_survey_id;
  get diagnostics n = row_count;
  delete from surveys where survey_id = p_survey_id;
  return n;
end;
$$;
grant execute on function public.admin_delete_survey(text, text) to anon;

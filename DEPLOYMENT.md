...
When a new version adds database fields or functions (e.g. canton, the
contact question, per-question monitoring, CSV export and delete), simply
**re-run the current `supabase/schema.sql`** in the SQL editor. The script is idempotent: it adds
missing columns, replaces the dashboard function, and leaves existing data and
access codes untouched.

## Export, deletion and personal data

The dashboard's **CSV download** returns all individual responses of a survey —
including the voluntary follow-up contact details (e-mail or postal address).
Anyone holding a survey's access code can download this data, so treat access
codes accordingly and handle exports according to your data-protection rules.
The **Delete all data** button (behind an are-you-sure confirmation) removes a
survey's responses irreversibly.

## Brand fonts

The pages use Akkurat Pro (licensed) with Bebas Neue (free, via Google Fonts).
Put your licensed webfont files into the `fonts/` folder named exactly
`AkkuratPro-Regular.woff2` and `AkkuratPro-Bold.woff2` — see `fonts/README.txt`.
Without them the site falls back to Helvetica/Arial automatically.

## Updating to v3.1

Re-run `supabase/schema.sql` (adds `admin_delete_survey` for removing
customers from the hub) and upload the updated `index.html`,
`dashboard.html`, `hub.html` plus the new `js/jspdf.umd.min.js` (bundled
library for the PDF results report).

## Updating to v3

Re-run `supabase/schema.sql` (idempotent). It extends the dashboard summary
with the overview statistics and per-segment means, and adds
`admin_set_survey` so customers can be created from the hub. Then upload the
updated `index.html`, `dashboard.html`, `public.html` and `hub.html`.

## One-time setup for v2

After re-running `supabase/schema.sql`, set the master code for the hub once:

    select set_admin_code('choose-a-strong-master-code');

Upload the two new files `public.html` and `hub.html` together with the
updated `index.html`, `dashboard.html` and `js/content.js`.
